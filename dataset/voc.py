...
class VOCDataTrainSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, base_size=(512, 2048), crop_size=(512, 512), scale=True, mirror=True, ignore_label=-1
        ...
        self.is_cutout = cutout_flag
        self.is_cutmix = cutmix_flag
        self.is_mosaic = mosaic_flag
    ...
    def generate_512_image_label(self, image, label):
        h, w, _ = image.shape
        h_w_min = min([h, w])
        f_scale = 512/h_w_min
    ...
    def __getitem__(self, index):
        ...
        name = datafiles["name"]
        if self.scale:
            image, label = self.generate_512_image_label(image, label)

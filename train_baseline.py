...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='LRseg',
                        help='model name')  
    parser.add_argument('--backbone', type=str, default='Railway-seg',
                        help='backbone name')
    ...
    parser.add_argument('--max-iterations', type=int, default=int(total/4*200), metavar='N',
    ...
    parser.add_argument('--save-per-iters', type=int, default=int(total/4),
                        help='per iters to save')
    parser.add_argument('--val-per-iters', type=int, default=int(total/4),
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'voc':
            train_dataset = VOCDataTrainSet(args.data, './dataset/list/voc_rail/train.txt', max_iters=args.max_iterations*args.batch_size,
                                          crop_size=args.crop_size, scale=True, mirror=True
                                            ,cutout_flag=False, cutmix_flag=False, mosaic_flag=False)
        ...
        self.model = Our_model(num_classes=2).to(self.device)
        with torch.no_grad():
            self.model.eval()
        ...
        for iteration,  (images, targets, _) in enumerate(self.train_loader):
            ...
            if self.args.aux:
                task_loss = self.criterion(outputs[0], targets) + \
                            0.4 * self.criterion(outputs[1], targets)
    ...
    def validation(self):
        ...
        for i, (image, target, filename)  in enumerate(self.val_loader):
            ...
            Iou_list = Iou.tolist()
            b_list.append(Iou_list[0])
            r_list.append(Iou_list[1])
        mean_pixacc = sum(pixacc_list) / len(pixacc_list)
        mean_miou = sum(miou_list) / len(miou_list)
        b_iou = sum(b_list) / len(b_list)
        r_iou = sum(r_list) / len(r_list)
        logger.info("Validation mean pixAcc: {:.4f}, mean mIoU: {:.4f}, Background IoU: {:.4f}, Road IoU: {:.4f}".format(mean_pixacc, mean_miou, b_iou, r_iou))

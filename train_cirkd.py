...
def parse_args():
    ...
    parser.add_argument('--student-model', type=str, default='None',
                        help='model name')                      
    parser.add_argument('--student-backbone', type=str, default='LRseg',
                        help='backbone name')
    parser.add_argument('--teacher-backbone', type=str, default='resnet18',
                        help='backbone name')
    parser.add_argument('--dataset', type=str, default='voc',
                        help='dataset name')
    parser.add_argument('--data', type=str, default='./dataset/voc/',
    ...
    parser.add_argument('--max-iterations', type=int, default=584*200, metavar='N',
    ...
    parser.add_argument('--save-dir', default='./save_model',
                        help='Directory for saving checkpoint models')
    parser.add_argument('--save-epoch', type=int, default=1,
                        help='save model every checkpoint-epoch')
    parser.add_argument('--log-dir', default='../runs/logs/',
                        help='Directory for saving checkpoint models')
    parser.add_argument('--log-iter', type=int, default=250,
                        help='print log every log-iter')
    parser.add_argument('--save-per-iters', type=int, default=584,
                        help='per iters to save')
    parser.add_argument('--val-per-iters', type=int, default=584,
                        help='per iters to val')
    parser.add_argument('--teacher-pretrained-base', type=str, default='None',
                        help='pretrained backbone')
    parser.add_argument('--teacher-pretrained', type=str, default='./save_model/deeplabv3_resnet18_voc_best_model.pth',
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'voc':
            train_dataset = VOCDataTrainSet(args.data, './dataset/list/voc_rail/train.txt', max_iters=args.max_iterations*args.batch_size,
                                          crop_size=args.crop_size, scale=True, mirror=True)
            val_dataset = VOCDataValSet(args.data, './dataset/list/voc_rail/val.txt')
        else:
        ...
        self.s_model = Our_model(num_classes=2).to(self.device)
        ...
        s_y = self.s_model(x)
        import numpy as np
        # print(len(t_y), t_y[0].shape, t_y[1].shape,t_y[2].shape)
        # print(len(s_y), s_y[0].shape, s_y[1].shape)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            # print(s_outputs[0].shape, t_outputs[0].shape)
            if self.args.aux:
    ...
    def validation(self):
        ...
        mean_pixacc = sum(pixacc_list)/len(pixacc_list)
        mean_miou = sum(miou_list)/len(miou_list)
        logger.info("Validation mean pixAcc: {:.3f}, mean mIoU: {:.3f}".format(mean_pixacc, mean_miou))
